...
def isStrictlyDominant(M):
    for i in range(len(M)):
        ...
        if diag <= suma:
            print(f"La fila {i} no cumple: |{M[i][i]}| <= {suma}")
            return False
...
def jacobiMethod(M, B, P0, n):
    # Calcular la solución real
    true_solution = np.linalg.solve(M, B)
    ...
    Iteration(P0)
    results["k"].append(0)
    results["x"].append(round(P0[0], 8))
    results["y"].append(round(P0[1], 8))
    results["z"].append(round(P0[2], 8))
    error = np.linalg.norm(true_solution - P0)
    results["distancia"].append(round(error, 8))

    # Iteraciones siguientes
    for it in range(1, n + 1):
        prev_iter = Iteration.iterations[-1].P
        P = []
        ...
        results["k"].append(it)
        results["x"].append(round(P[0], 8))
        results["y"].append(round(P[1], 8))
        results["z"].append(round(P[2], 8))
        error = np.linalg.norm(true_solution - P)
        results["distancia"].append(round(error, 8))
...
def run():
    ...
    # Entrada del numero de iteraciones deseadas.
    n = int(input("Ingrese el numero de iteraciones: "))

    # Entrada del vector inicial.
    entrada = input("Ingrese la solución inicial (separada por espacios): ")
    P0 = np.array([float(x) for x in entrada.split()])

    # Llevar a cabo el metodo con las condiciones dadas.
    init=time.perf_counter() #iniciamos a medir el tiempo
    # aqui se ejecuta y se guarda el diccionario que retorna jacobimethod
    results = jacobiMethod(M, B, P0, n) #terminamos de medir el tiempo
    end=time.perf_counter()
    print(getTabla(results, setTitle="Iteraciones del Método de Jacobi", character=" "))
